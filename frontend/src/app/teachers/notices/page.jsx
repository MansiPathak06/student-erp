...
function NoticeCard({ n }) {
  ...
  return (
    <div className={`rounded-2xl border transition-all duration-200 overflow-hidden
      ${isUrgent ? "border-red-200 bg-red-50/40" : "border-gray-100 bg-white hover:border-emerald-200 hover:shadow-sm"}`}>
      ...
      <button onClick={() => setOpen(o => !o)}
        ...
        <ChevronDown size={16} className={`flex-shrink-0 mt-1.5 text-gray-400 transition-transform duration-200 ${open ? "rotate-180" : ""}`} />
      </button>

      {open && (
        <div className="px-5 pb-5 border-t border-gray-100">
          <p className="mt-4 text-sm text-gray-700 leading-relaxed whitespace-pre-wrap bg-gray-50 rounded-xl p-4 border border-gray-100">
            {n.content}
          </p>
        </div>
      )}
        ...
        <div className="bg-white border-b border-gray-100 px-4 sm:px-6 py-4 shadow-sm">
          <div className="pl-10 lg:pl-0 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-gray-400 mt-0.5">All announcements for teachers</p>
            </div>
            <button onClick={load}
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-5">
              ...
              {ALL_CATEGORIES.map(c => (
                <button key={c} onClick={() => setCategory(c)}
                  className={`text-xs font-semibold px-3 py-1.5 rounded-full transition-colors
                    ${category === c ? "bg-emerald-600 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}>
                  {c}
                </button>
              ))}
            </div>
          ...
          {!loading && pinned.length > 0 && (
            <div>
              ...
              <div className="space-y-3">{pinned.map(n => <NoticeCard key={n.id} n={n} />)}</div>
            </div>
          )}

          {!loading && unpinned.length > 0 && (
            <div>
              {pinned.length > 0 && <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-2 px-1">All Notices</p>}
              <div className="space-y-3">{unpinned.map(n => <NoticeCard key={n.id} n={n} />)}</div>
            </div>
          )}

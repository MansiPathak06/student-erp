  ...
  try {
    ...
    const result = await pool.query(
      `SELECT tt.*, u.name as teacher_name, c.class_name, c.section
       ...
       JOIN users u ON t.user_id = u.id
       WHERE c.class_name=$1 AND c.section=$2
       ORDER BY CASE day_of_week WHEN 'Monday' THEN 1 WHEN 'Tuesday' THEN 2 WHEN 'Wednesday' THEN 3 WHEN 'Thursday' THEN 4 WHEN 'Friday' THEN 5 WHEN 'Saturday' THEN 6 END, tt.start_time`,
      [cls, section]
    );
    res.json(result.rows);
  } catch (err) {
...
module.exports = { getProfile, getAttendance, getResults, getFees, getAssignments, getTimetable };